...
class CBitcoinAddressError(bitcoinlib.base58.Base58Error):
    """Raised when an invalid Bitcoin address is encountered"""

class CBitcoinAddress(bitcoinlib.base58.CBase58Data):
    """A Bitcoin address"""
    ...
    def from_bytes(cls, data, nVersion):
        ...
        if nVersion == bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']:
            self.__class__ = P2SHBitcoinAddress

        elif nVersion == bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR']:
...
class P2SHBitcoinAddress(CBitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']

        elif nVersion != bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']:
    ...
    def from_scriptPubKey(cls, scriptPubKey):
        ...
        if scriptPubKey.is_p2sh():
            return cls.from_bytes(scriptPubKey[2:22], bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR'])
    ...
    def to_scriptPubKey(self):
        """Convert an address to a scriptPubKey"""
        assert self.nVersion == bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']
...
class P2PKHBitcoinAddress(CBitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR']
    ...
    def from_pubkey(cls, pubkey, accept_invalid=False):
        """Create a P2PKH bitcoinlib address from a pubkey
        ...
        if not accept_invalid:
            if not isinstance(pubkey, bitcoinlib.core.key.CPubKey):
                pubkey = bitcoinlib.core.key.CPubKey(pubkey)
            if not pubkey.is_fullyvalid:
                raise CBitcoinAddressError('invalid pubkey')

        pubkey_hash = bitcoinlib.core.Hash160(pubkey)
    ...
    def from_scriptPubKey(cls, scriptPubKey, accept_non_canonical_pushdata=True, accept_bare_checksig=True):
        ...
        if accept_non_canonical_pushdata:
            ...
            except bitcoinlib.core.script.CScriptInvalidError:
        ...
        if (len(scriptPubKey) == 25
            ...
            return cls.from_bytes(scriptPubKey[3:23], bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR'])
    ...
    def to_scriptPubKey(self):
        """Convert an address to a scriptPubKey"""
        assert self.nVersion == bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR']
...
class CKey(object):
    ...
    def __init__(self, secret, compressed=True):
        self._cec_key = bitcoinlib.core.key.CECKey()
        self._cec_key.set_secretbytes(secret)
        self._cec_key.set_compressed(compressed)

        self.pub = bitcoinlib.core.key.CPubKey(self._cec_key.get_pubkey(), self._cec_key)
...
class CBitcoinSecretError(bitcoinlib.base58.Base58Error):
    pass

class CBitcoinSecret(bitcoinlib.base58.CBase58Data, CKey):
    ...
    def from_secret_bytes(cls, secret, compressed=True):
        ...
        self = cls.from_bytes(secret + (b'\x01' if compressed else b''),
                              bitcoinlib.params.BASE58_PREFIXES['SECRET_KEY'])
        self.__init__(None)
        return self

    def __init__(self, s):
        if self.nVersion != bitcoinlib.params.BASE58_PREFIXES['SECRET_KEY']:
            raise CBitcoinSecretError('Not a base58-encoded secret key: got nVersion=%d; expected nVersion=%d' % \
                                      (self.nVersion, bitcoinlib.params.BASE58_PREFIXES['SECRET_KEY']))
